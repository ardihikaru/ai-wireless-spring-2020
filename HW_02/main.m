...
num_ignored_epoch = 0;
ignored_epoch = [1, 2, 3, 4, 5, 6, 7, 11, 14, 21, 33, 34, 35, 36];  % for some reasons this epoch always produce a very low APC score
epoch = 0;
C_APC = [];
...
for time_slot = 1 : runtime 
    ...
    if ismember(time_slot, ignored_epoch)
        num_ignored_epoch = num_ignored_epoch + 1;
    else
        % perform cluster center optimization here
        epoch = epoch + 1;
        ...
        [SINR_APC(epoch,:), datarate_APC(epoch,:), C_APC(epoch,:)] = All_Capacity(Rx_signal_APC, Rx_interfer_APC, ue_num, W, N0);
        SINR_APC_dB(epoch,:) = 10*log10(SINR_APC(epoch,:));
    end
%     n(cluster_center) = 0; % n: the # of served users per cell
%     bs_loc_intuitive_APC = bs_loc_intuitive(n ~= 0);
%     Rx_power_APC = RSRP_total_intuitive(:,n ~= 0);
...
C_enhancement = ((C_APC_avg - C_all_on_avg) / C_all_on_avg) * 100;
